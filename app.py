...
6. 修复：默认到期日索引逻辑，避免选中下月到期日（核心修复点）
"""
...
with st.sidebar:
    ...
    # 修正默认到期日逻辑（核心修复点）：优先选中当月未过期的到期日
    today = date.today()
    this_month_wed = get_fourth_wednesday(today.year, today.month)
    # 逻辑修正：如果当月到期日未过期，默认选第一个（index=0）；否则选下一个（index=1）
    if len(expiry_options) == 0:
        default_idx = 0
    elif today <= this_month_wed:
        default_idx = 0  # 当月到期日未过期，选第一个
    else:
        # 当月到期日已过期，选下一个（index=1，若存在）
        default_idx = 1 if len(expiry_options) > 1 else 0
    
    selected_expiry = st.selectbox(
        "选择到期日期 (每月第四个星期三)", 
        options=expiry_options, 
        index=min(default_idx, len(expiry_options)-1)  # 防止索引越界
    )
    # 到期时间固定为15:00（A股收盘）
